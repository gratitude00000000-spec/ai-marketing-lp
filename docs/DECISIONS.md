...
### D-024 印章デザインリニューアル（design/stamp-restyle）を main にマージし本番公開
- **判断:** `design/stamp-restyle` ブランチ（ヒーロー/ステータスバー3D演出、対応業種3Dマルキー、料金プランのスマホ横スワイプカルーセル、サービス内容セクション刷新＋スマホ用メインカード+サムネイルカルーセル、会社概要・お問い合わせページの整理、ヘッダーのハンバーガー配置バグ修正等、計32コミット）をユーザー承認のうえ `main` に `--no-ff` マージし、`npx netlify deploy --prod --build` で本番公開した。
- **理由:** ユーザーから「とりあえずここまでの編集でOK、公開してみよう」と明示承認を得た。事前にローカル `npm run build` で型チェック・ビルドが通ることを確認済み。
